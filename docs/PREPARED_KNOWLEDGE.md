...
4. 事前入力されたtitle、summary、type、keywordsを確認する
5. 保存先付きMarkdownプレビューで本文とfront matterを確認する
6. `Ctrl+S`で保存する
...
`prepared_knowledge`がない通常テキストでは、従来どおりtitleの初期値だけを作り、summary、type、keywordsと本文を人が入力します。構造化済みを宣言した入力に不正なtypeや不足見出しがある場合は、空テンプレートへfallbackせず登録を中止します。
