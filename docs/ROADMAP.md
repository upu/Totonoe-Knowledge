...
現在の状態: M1〜M3のコードと自動テストは実装済み。M4の実データdogfoodingが完了するまで、v0.1は未完了として扱う。
...
現在の状態: Save/Search Toolと保存確認、Extension Host統合テスト、Markdown整合性検査を実装済み。実Copilot AgentでのTool参照確認と、v0.1 dogfoodingで得た改善の反映が残っている。
