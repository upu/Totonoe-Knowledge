...
- [ ] 既存Entryと検索語、type、適用version、keywordsが重なる新規Entryでは、保存先付きプレビューの前に関係候補の確認を選べる
- [ ] 候補は上位数件だけで、Duplicate / Related / Complement / Conflict / Supersede、推定理由、根拠EntryのID・タイトルを表示する
- [ ] `consolidates`を持つ既存Current Viewも対象範囲が重なる場合に候補へ表示する
- [ ] 候補を選ぶと根拠Entryを開けるが、この段階では新規・既存Markdownへ関係を書き込まない
- [ ] `省略して続ける`を選ぶと候補確認なしで従来の保存先付きプレビューへ進む
- [ ] 候補なし、Language Modelなし、分類失敗では関係を作らず従来の登録を続行する
- [ ] 候補ごとに採用、関係種別の編集、却下を選べる
- [ ] Duplicate採用時は新規登録の中止または既存Entryの編集へ分岐し、自動統合しない
- [ ] Supersedeは新Entryの`supersedes`、Complement / Relatedは`related`、Conflictは双方の`conflicts`として差分表示される
- [ ] Current View更新案は本文とfront matterの両方をMarkdown差分で確認できる
- [ ] 差分確認画面と登録プレビューで編集した内容が、そのまま保存される
- [ ] 候補却下、途中キャンセル、既存Markdownの競合時に未承認の新規・既存Markdown変更が残らない
- [ ] 候補検索と差分確認の前後で`.totonoe/index.sqlite`と`.totonoe/vectors/index.json`が変わらない

## Current View

- [ ] `Generate or Update Current View`で複数の粒Entryを選び、`type: specification`、`consolidates`、`consolidated_at`を持つ保存前プレビューが開く
- [ ] 生成前にLanguage Modelを選べ、秘密情報候補がある根拠を外部送信する前と、生成案をローカルプレビューで開く前に確認が表示される
- [ ] 既存Current Viewの再生成で従来の`id`、`created_at`、適用version、関係を保持し、Current Viewと根拠Entryの編集可能な差分を承認後だけ保存する
- [ ] `Show Current View Links`でCurrent Viewから全根拠Entryを開き、各Entryの本文と元情報を確認できる
- [ ] 粒Entryから`Show Current View Links`を実行すると、そのEntryを`consolidates`するCurrent Viewを逆引きできる
- [ ] `Mark Entry as Affecting Current View`で粒Entryの`affects`へ対象を追加できる
- [ ] 再生成で`affects`対象を根拠へ加え、承認後にCurrent Viewの`consolidates`へ追加すると粒Entry側の`affects`が除去される
- [ ] 再生成や関係更新をキャンセルした場合、Current Viewと粒Entryのどちらも変更されない
...
- [ ] Current Viewの未知・重複`consolidates`と不正な`consolidated_at`をエラーとして表示する
- [ ] 根拠Entryの後更新、未包含Entryへの置き換え、未解消`affects`を陳腐化警告として表示する
