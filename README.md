...
ビルド済みの`dist/mcp-server.js`は、Codex desktop app / CLI / IDE extensionから利用できるread-onlyのstdio MCPサーバです。`totonoe_knowledge_search`と`totonoe_knowledge_get`だけを公開し、Repository rootは起動時に固定します。VS CodeのLanguage Model Toolと同じ検索coreを使うため、GitHub Copilotを呼ばずに同じ全文・metadata・意味検索順位を利用できます。
...
`npm run package` は型チェック、70件以上のユニットテスト、Extension Host / MCP向けバンドル、実stdio MCP clientによる検索・取得・fallback dogfoodingを実行します。`npm run test:integration` はVS Code Stableを起動し、activate・コマンド・Tool・Problems診断・SQLiteインデックス再構築を確認します。VS Codeテスト環境の取得だけは一時的な通信失敗に備えて最大3回再試行し、起動後のテスト失敗は再試行しません。Pull Requestと`main`へのpushでも両方をGitHub Actionsで実行します。
...
- Codex向けread-onlyローカルstdio MCPと固定Repository境界
- VS Code / MCPで共有する検索core、順位回帰fixture、実stdio dogfooding
