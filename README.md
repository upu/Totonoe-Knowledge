...
検索インデックスはワークスペース利用時はワークスペースの `.totonoe/index.sqlite`、外部リポジトリ利用時は選択したフォルダーの `.totonoe/index.sqlite` に作られ、Git管理対象外です。Markdown変更は次の検索時にファイルの更新時刻とサイズで判定し、追加・変更・削除分だけ反映します。任意の時点で `Rebuild Search Index` を実行して全Markdownから作り直せます。詳細は [docs/SEARCH_INDEX.md](docs/SEARCH_INDEX.md) を参照してください。
