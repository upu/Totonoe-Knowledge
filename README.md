...
ビルド済みの`dist/mcp-server.js`は、Codex desktop app / CLI / IDE extensionから利用できるローカルstdio MCPサーバです。Repository rootは起動時に固定し、read-onlyの`totonoe_knowledge_search` / `totonoe_knowledge_get`と、確認付き書き込みの`totonoe_knowledge_preview_registration` / `totonoe_knowledge_register`を公開します。検索はVS CodeのLanguage Model Toolと同じcoreを使用します。登録はCodexが作った固定schemaの内容だけを受け取り、GitHub Copilotや別のLLMをサーバ内から呼びません。

登録は必ずpreviewとregisterの二段階です。previewが返すtitle、Repository相対path、secret finding、canonical Markdown diffを確認してから、同じpayloadと短命one-time tokenでregisterします。preview後にRepositoryが変わった場合、tokenを再利用した場合、保存先が既にある場合は上書きせず拒否します。

設定例、Ollamaオプション、出力上限、dogfooding手順は [docs/CODEX_MCP.md](docs/CODEX_MCP.md) を参照してください。
...
`npm run package` は型チェック、70件以上のユニットテスト、Extension Host / MCP向けバンドル、実stdio MCP clientによる検索・取得・確認付き登録・fallback dogfoodingを実行します。`npm run test:integration` はVS Code Stableを起動し、activate・コマンド・Tool・Problems診断・SQLiteインデックス再構築を確認します。VS Codeテスト環境の取得だけは一時的な通信失敗に備えて最大3回再試行し、起動後のテスト失敗は再試行しません。Pull Requestと`main`へのpushでも両方をGitHub Actionsで実行します。
...
- Codex向けローカルstdio MCPのread Toolと確認付きregistration Tool
- VS Code / MCPで共有する検索・登録core、順位回帰fixture、実stdio dogfooding
...
ローカルstdio MCPは実装済みですが、チーム共有用のHTTP Repository / Remote MCPはまだ未実装です。社外秘データを扱う前提の認証・認可・Tool出力・監査境界は [docs/REMOTE_SECURITY_MODEL.md](docs/REMOTE_SECURITY_MODEL.md)、API契約は [docs/REMOTE_REPOSITORY_API.md](docs/REMOTE_REPOSITORY_API.md)、可読バックアップと復旧手順は [docs/BACKUP_RESTORE.md](docs/BACKUP_RESTORE.md) に先行して定義しています。
